...
impl<'a> Button<'a> {
    pub fn new(coords: (f64, f64), width: Option<f64>, font: &'a Font, text: String) -> Button<'a> {
        let mut text = Text::new_with_text_and_coords(
            &font,
            text,
            (coords.0 as usize + 7, coords.1 as usize + 35),
        );
    ...
    pub fn _is_pressed(&self) -> bool {
        ...
        mouse_position.0 > self.coords.0
            ...
            && mouse_position.1 < self.coords.1 + 50.0
            && is_mouse_pressed()
    }
}
...
impl<'a> Drawable for Button<'a> {
    fn draw_on_canvas(&self, mut canvas: &mut Canvas) {
        ...
        if mouse_position.0 > self.coords.0
            && mouse_position.0 < self.coords.0 + width
            && mouse_position.1 > self.coords.1
            && mouse_position.1 < self.coords.1 + 50.0
        {
