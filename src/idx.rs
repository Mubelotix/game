...
impl HexIndex {
    ...
    pub fn neighbors_present(&self) -> (bool, bool, bool, bool, bool, bool) {
        (
            self.top_right_bloc_present(),
            self.right_bloc_present(),
            self.bottom_right_bloc_present(),
            self.bottom_left_bloc_present(),
            self.left_bloc_present(),
            self.top_left_bloc_present(),
        )
    }
            ...
            if self.pos.1 <= 4 {
                Some(HexIndex {
                    pos: (
                        self.pos.0,
                        self.pos.1 - 1,
                        self.pos.2 - (line_lenght(self.pos.1) - 1),
                    ),
                })
            } else {
                Some(HexIndex {
                    pos: (
                        self.pos.0 + 1,
                        self.pos.1 - 1,
                        self.pos.2 - line_lenght(self.pos.1),
                    ),
                })
            }
        } else {
...
mod test {
    ...
    fn test_neighbors() {
        ...
        assert_eq!(
            top_top_left.neighbors_present(),
            (false, true, true, true, false, false)
        );
        assert_eq!(
            top_left.neighbors_present(),
            (true, true, true, true, false, false)
        );
        assert_eq!(
            left.neighbors_present(),
            (true, true, true, false, false, false)
        );
        assert_eq!(
            bottom_left.neighbors_present(),
            (true, true, true, false, false, true)
        );
        assert_eq!(
            bottom_bottom_left.neighbors_present(),
            (true, true, false, false, false, true)
        );

        assert_eq!(
            top_top_right.neighbors_present(),
            (false, false, true, true, true, false)
        );
        assert_eq!(
            top_right.neighbors_present(),
            (false, false, true, true, true, true)
        );
        assert_eq!(
            right.neighbors_present(),
            (false, false, false, true, true, true)
        );
        assert_eq!(
            bottom_right.neighbors_present(),
            (true, false, false, true, true, true)
        );
        assert_eq!(
            bottom_bottom_right.neighbors_present(),
            (true, false, false, false, true, true)
        );

        assert_eq!(
            top.neighbors_present(),
            (false, true, true, true, true, false)
        );
        assert_eq!(
            middle.neighbors_present(),
            (true, true, true, true, true, true)
        );
        assert_eq!(
            bottom.neighbors_present(),
            (true, true, false, false, true, true)
        );

        assert_eq!(
            top_top_left
                .get_bottom_left_neighbour()
                .unwrap()
                .get_bottom_left_neighbour()
                .unwrap(),
            top_left
