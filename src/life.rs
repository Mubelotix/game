...
impl Life {
    ...
    pub fn draw_on_canvas(&self, mut canvas: &mut Canvas, data: &DrawingData) {
        ...
        let coords = Map::internal_coords_to_screen_coords(data.dimensions, data.margin, coords.0 as isize + (256 - width as isize) / 2, coords.1 as isize + 300);
        let context = canvas.get_2d_canvas_rendering_context();
        context.begin_path();
        context.stroke_rect(coords.0 as f64, coords.1 as f64, width as f64 * data.factor, height as f64 * data.factor);
        context.set_fill_style(&JsValue::from_str("rgb(24, 28, 39)"));
        context.fill_rect(coords.0 as f64, coords.1 as f64, width as f64 * data.factor, height as f64 * data.factor);

        context.set_fill_style(&JsValue::from_str("rgb(0, 255, 100)"));
        for i in 0..self.current {
            context.fill_rect(coords.0 as f64 + (4.0 + i as f64 * point_width) * data.factor, coords.1 as f64 + 4.0 * data.factor, (point_width - 4.0) * data.factor, height as f64 * data.factor - 8.0 * data.factor);
        }
        context.stroke();
    }
}