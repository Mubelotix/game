...
use wasm_game_lib::graphics::{canvas::*, color::Color, drawable::*};

pub fn compute_travel_time(
    units: &Units,
    _map: &Map,
    starting_point: HexIndex,
    max_moves: usize,
) -> [Option<usize>; 61] {
...
pub fn find_route(
    travel_time: &[Option<usize>; 61],
    starting_point: HexIndex,
    arrival_point: HexIndex,
) -> Option<Vec<HexIndex>> {
    if travel_time[arrival_point.get_index()].is_some() {
        ...
        while last != starting_point {
            ...
            let neighbours = [
                last.get_left_neighbour()
                    .map(|n| (travel_time[n.get_index()], n)),
                last.get_right_neighbour()
                    .map(|n| (travel_time[n.get_index()], n)),
                last.get_top_left_neighbour()
                    .map(|n| (travel_time[n.get_index()], n)),
                last.get_top_right_neighbour()
                    .map(|n| (travel_time[n.get_index()], n)),
                last.get_bottom_left_neighbour()
                    .map(|n| (travel_time[n.get_index()], n)),
                last.get_bottom_right_neighbour()
                    .map(|n| (travel_time[n.get_index()], n)),
            ];
            let mut neighbours: Vec<(usize, &HexIndex)> = neighbours
                .iter()
                .filter(|v| v.is_some())
                .map(|v| v.as_ref().unwrap())
                .collect::<Vec<&(Option<usize>, HexIndex)>>()
                .iter()
                .filter(|(t, _i)| t.is_some())
                .map(|(t, i)| (t.unwrap(), i))
                .collect::<Vec<(usize, &HexIndex)>>();
